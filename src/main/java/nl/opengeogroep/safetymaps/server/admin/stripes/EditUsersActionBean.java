...
public class EditUsersActionBean implements ActionBean, ValidationErrorHandler {
    ...
    private void loadInfo() throws NamingException, SQLException {
        allRoles = qr().query("select role, coalesce(description, role) as description from " + ROLE_TABLE + " where protected = false or (protected = true and role = 'admin') order by protected desc, role", new MapListHandler());
    ...
    public Resolution save() throws Exception {
        ...
        qr().update("delete from " + USER_ROLE_TABLE + " where username = ? and (protected = false or left(role, 6) = 'smvng_')", username);
